...
- **競馬について対話する依頼**では、Codex が依頼の完了に必要な事前調査を行い、調査結果を
  タスクMarkdownへ記録してから Claude Code へ渡す。Claude Code は資料と DB を先に使い、不足や
  誤りの疑いが判断に影響するときは追加調査してよい。複数の分析する役が必要な依頼は、
  オーケストレーターとして実行し、1つの専門役だけに限定しない
