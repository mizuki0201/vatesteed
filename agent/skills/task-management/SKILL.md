...
貼らない。**競馬では、確認した事実をDBへ登録して読み直すところまでを進行役が終える。** その結果を
...
`事実の登録結果` には、進行役がDBへ登録して読み直した事実を書く。**ここが空のままでは
`pnpm claude:opus` が競馬タスクを起動しない。** `未登録・未分析` には、実行元が進行役へ返す
不足を、対象・必要な事実・その事実を使う判断の3つで書く。
...
5. 準備が終わるまでは `preparation_status: preparing` にする。進行役が必要な事前調査を終え、
   「事前調査」と（競馬では）「参照元」「事実の登録結果」を埋めてから `ready` にする
6. 共通項目とモード別の項目を埋める
