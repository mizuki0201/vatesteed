...
- **eve の構成でプロジェクトを作る**が、Phase 1 の Codex から Claude Code へ渡す経路では、
  Codex が人間との対話・外部情報の収集・事実のDB登録・文章確認、Claude Code が分析・予想と
  その結果の保存を担う。Vatesteed 自体を作る依頼では Claude Code が実装を担う。
  **この配置は暫定であり固定しない**。本人が実行元を指定した場合はその指示を優先する
...
共通のタスク形式は、次の3通りで使う。Codex から Claude Code を呼ぶ経路だけを共通ルールにしない。

| 進行役 | 実行元 | 使い方 |
| --- | --- | --- |
| Codex | Claude Code | Codex が準備したタスクMarkdownを、リポジトリの `pnpm claude:opus` から渡す |
| Codex | Codex | 本人が Codex 単体を指定した場合に、Codex が準備・実行・確認を順番に行う |
| Claude Code | Claude Code | Claude Code を直接起動した場合に、同じ実行元が準備・実行・確認を順番に行う |

実行元と、`dev-implementer`、`entry-analyst` などの役は別の情報として記録する。複数の競馬の役が
必要な依頼では、実行時の役をオーケストレーターとし、必要な専門役の指示をタスク本文で指定する。
実行元の違いで、役の定義や競馬について対話する運用を変えない。
...
| 陣営・騎手コメント（1出走あたり2件・200字。[`entry_comments`](data-model.md#コメント)） | 96,000 | 0.6 KB | 約 58 MB |
...
- **蓄積型**: 馬・騎手・血統・陣営コメント。走るたびに追記され、履歴が消えると価値が落ちる。
  **コメントだけは実際に履歴として持つ**（[data-model.md](data-model.md#コメント)）。評価は
  1対象1行の上書きなので、蓄積型といっても履歴は残らない
