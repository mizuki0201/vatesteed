...
Claude Code を誰が呼ぶかは固定しない。Codex から呼ぶときはプロジェクトの MCP を使い、
Claude Code 単体で使うときは、その環境の認証付き起動方法を使う。どちらの場合も、開発役だけに
限らず、生成済みの役から依頼に合うものを選ぶ。実行元の違いで、役の定義や競馬について対話する
運用を変えない。
