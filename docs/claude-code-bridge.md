...
ファイルからの置き換えで行い、途中で終了しても半分だけ書かれたJSONを残さない。接続確認と、
この仕組みを入れる前の実行記録は対象にしない。
...
**成功した接続確認は、そのCodexタスクが終わるまで使い回す。** 本来の依頼の前、再開の前、進捗確認の
たびに繰り返さない。接続確認が成功したあとで本実行がAPIエラーになっても、認証確認をやり直さず、
同じ実行記録の状態とエラーを基準に扱う。
