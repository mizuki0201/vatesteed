...
| 購入（5） | `ai_bets` `ai_bet_legs` `ai_bet_rationales` `my_bets` `my_bet_legs` |
| 確定払戻（1） | `race_payouts` |
| 閲覧権限（1） | `users` |
...
2026-08-22 に、`ai_bet_rationales` を 2026-09-19 に足している。
...
- **今回の分析対象に直接属する事実だけを登録する。** 調査で参照した関連情報を一括して入れない
- 馬の過去のレースでは対象馬の出走だけを登録し、他の出走馬と払戻は調査資料だけで扱う
- 血統分析では先祖を調べても、対象馬の分析に付随するだけなら先祖の馬の行を作らない
- コースの形のように、別の分析でも使うことが明白で時点に依存しない一般情報は登録してよい
...
**日本の競馬場（中央10場と、ばんえいを除く地方14場）は、どれも最低1つのコースを登録してある**
（2026-09-17）。画面からコースを登録するとき、競馬場名を登録済みのものから選べるようにして、
「阪神」「阪神競馬場」のような揺れで同じコースが2つできるのを防ぐため。帯広（ばんえい）は
`surface` に当てはまる値が無いので入れていない。
...
| `is_overseas` | boolean | 海外の馬なら true。既定は false。null 不可 |
| `created_at` `updated_at` | timestamptz | |
...
確認し、以後も定期的に確認して更新する。**引退したことは分かっても日付が分からない馬は
`1900-01-01` を入れる。** 画面では「日付不明」と表示する。**馬の詳細画面からも owner が切り替えられる。**
引退に変えるときは確認画面で引退日を入れられ、空のままなら `1900-01-01` になる。現役に戻すと null にする。

**海外の馬は `is_overseas` で分ける。** 一覧の表示区分は「すべて」「現役」「引退」「海外」の4つで、
海外の馬は現役にも引退にも出さない。引退を確認する手順は日本の馬を前提にしていて、海外の馬には
同じ確認ができず、`retired_at` が null のまま現役に混ざるため。**海外の馬をさらに現役と引退に
分けることは、今はしない。**

判定は登録するときに決めて列に持ち、画面では毎回名前から判定しない。既に登録してある馬は、
`0012_horses_is_overseas.sql` で馬名に ASCII のローマ字を含むものを海外にした（本番DBで326頭中30頭）。
...
- **レース後** — `race_notes` と `entry_notes` を書いたあと、出走した各馬の `horse_notes` を
  読み直す。レースと1つの出走の評価では複数の出走を総合せず、馬のまとめだけは今回の
  `entry_notes` によって結論が変わる項目を更新する。変更対象外の項目は書き換えない
- **予想の会話の中** — 予想するレースの出走馬について過去の出走を振り返り、`horse_notes` を
  更新する

`entry_notes` に次の出走での扱いを書くのは、人間がそのレースの解釈として明示したときだけ。
AI は自分から次の扱いを作らない。
...
レース後の `horse_notes` も同じ分担で、`horse-analyst` は変更する項目と理由を返すだけにする。
`verifier` が変更前と比べたあと、オーケストレーターが項目単位で反映する。`author` が「人間」
「対話」なら本人との合意まで更新しない。本文の分け方は
...
- 展開と印は**枠順確定直後**に出す。記事にするのもこの展開と印
- 買い目は**レース当日の午前**に、その時点のオッズを見て組む（2026-09-19 決定 →
  [agent-design.md](agent-design.md#いつ作るか2026-09-19-決定)）
- **回収率は、当日の午前に組んだ買い目で測る。** 記事に出した内容と成績を一致させるため。
  買い目を組むときに展開と印は書き直さないので、回収率は枠順確定後の展開と印の上に乗る
...
**AI の買い目は、どれが当たっても払戻がほぼ同じになるように金額を割る**（2026-09-19 決定。
理由は [agent-design.md](agent-design.md#5-買い目を組む)）。配当の低い組み合わせに厚く、高い
組み合わせに薄く置くので、**組み合わせごとに金額が違う。**

- `ai_bets` は1行につき1点あたりの金額（`unit_amount`）を1つしか持てないので、**金額が違う
  組み合わせは1組み合わせを1行（単点）で入れる。** 列の持ち方はそのまま使える
- 割り方は `lib/bets` で出す。予算を100円単位で、いま最も払戻が小さい組み合わせへ順に足していく。
  **予算は余らせない**
- **どれが当たっても払戻が予算を上回るか**も同じところで判定する。上回らない組み合わせの集まりは
  買わない。判定に使うのは当日の午前のオッズで、ワイドと複勝のように幅で出るものは下限を使う
- 払戻は JRA と同じく、100円あたりの払戻（オッズ×100円）に、買った口数（金額÷100円）を掛けて出す

### `ai_bet_rationales`

**AI の買い目を、なぜその形にしたか**（2026-09-19 追加）。1レースにつき1行。

**画面にはまだ出していないが、出しても意味が通る文章で書く。** 評価の本文と同じく、保存先の
名前・役の名前・作業手順の説明は入れない（→
[agent-design.md の 本文に出さないもの](agent-design.md#本文に出さないもの)）。

| カラム | 型 | 内容 |
| --- | --- | --- |
| `id` | bigserial | PK |
| `race_id` | bigint | 一意。FK → `races` |
| `odds_checked_at` | timestamptz | **いつ時点のオッズで組んだか** |
| `body` | text | 買い目の理由。下を参照 |
| `created_at` `updated_at` | timestamptz | |

`body` に書くもの:

- 主に買う券種と、それを選んだ理由（印と展開のどこから選んだか）
- 印の順位と、その時点の人気の順位の並び。**どの馬の印の順位が人気より上だったか**
- 買った組み合わせごとのオッズ・金額・当たったときの払戻
- 候補にしたが買わなかった組み合わせと、その理由（当たっても払戻が予算を上回らない、など）

**`ai_bets` の列にせず、別のテーブルにする。** `ai_bets` は1組み合わせごとの行になることが
多く（→ [金額の割り方](#金額の割り方)）、理由はレースごとのものなので、列にすると同じ文章が
行の数だけ並ぶ。

- **予想時点の評価とは分ける。** 印の理由（`ai_predictions.rationale`）は枠順確定後に固定して
  あり、買い目の理由は当日の午前に書く。時点が違うので混ぜない
- **組み直すときは、`ai_bets` と一緒に入れ直す。** `ai_bets` を消す操作は人間に断ってから
  行う（→ [ai_bets](#ai_bets--my_bets)）
- 2026-09-18 以前の買い目には行が無い。この仕組みを入れる前に組んだもので、後から理由を作って
  足さない
- 自分（人間）の買い目（`my_bets`）には対応するテーブルを置かない。人間の買い方の理由は、
  人間が必要なときに対話で残す
...
- 入れるのは、対象レースの振り返りまたは買い目の答え合わせで必要な場合だけ。馬の過去の出走を
  分析するためにそのレースを参照しただけなら入れない
