...
2026-08-08 のブレストで、スキーマの叩き台まで作った。**まだ実際に `CREATE TABLE` は流していない。**
マイグレーション運用が未確定のため（下記「未確定事項」）。
...
| `trainer_id` | bigint | 現在の所属。FK → `trainers` |
| `created_at` `updated_at` | timestamptz | |
...
- UNIQUE: `(bet_id, leg_group, 選択した対象)`
...
- **マイグレーション運用** — ツール、適用タイミング、ロールバックの方針。
  **ORM（Drizzle など）を使うかどうかもここに含む。** 採るとスキーマ定義が TypeScript になるため、
  書き方そのものが変わる。**別セッションで扱う**
  - Neon の Preview ブランチを無効にしているため、**本番DBのコピーで試す手段が今は無い**
- `lib/` の切り方と、`expandBet` / `isHit` の置き場所 → [agent-design.md](agent-design.md)
- ワイドに、複勝のような出走頭数による着順の縮小があるかどうか。JRA の公式ページに記載を
  見つけられなかった。**現時点のコードは常に3着までとして書いている**
- 取消・除外の返還ルールの詳細（`refund` に何を入れるか）
