...
/**
 * `memos.status`。**外で見かけた話を、取り込むまでの間どう扱っているか。**
 *
 * 「保留」は宛先がまだ無いもの。その出走が DB に入るのは枠順が確定してからなので、
 * **保留は例外ではなく通常の経路**にあたる（docs/data-model.md#保留は例外ではない）。
 */
export const MEMO_STATUSES = ["未処理", "取り込み済み", "見送り", "保留"] as const;
